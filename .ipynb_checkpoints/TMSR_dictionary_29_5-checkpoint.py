...
far_right = ["(far.|extreme.|alt.)?right(ist|wing)?\\b(?!.*brain)", "nazi", "\\bvox\\b", "\\bafd\\b", "\\bpegida\\b", "\\bgolden.dawn\\b", "\\bnpd\\b", "identitarian.movement"]
...
environmentalism = ["\\benvironmental.+(cause|extremi|radical|criminology)?", "climate protest", "ecology", "environment", "\\beco(?!n).*(radical|terror)","ecologically motivated violence"]
...
deradicalization = ["\\bderadicali[zs]ation\\b", "^disengagement$"]
...
significance_quest = ["significance(.quest|loss)?", "quest.for.significance", "\\bself.esteem\\b", "\\bqfs\\b", "\\bneed.for.status\\b"
                      , "\\bneed.to.belong\\b", "\\b(need|search).for.meaning\\b", "^meaning$", "meaning.+ (in)?life", "\\bquest.for.(personal).?meaning",
                      "\\bloss.of.(meaning|significance)\\b", "\\bstatus.seeking\\b", "\\bbelonging\\b"]
...
mortality_salience = ["mortality salience", "death.anxiety", "fear of death"]
...
nationalism = ["nationali", "ethnocentri", "\\bnational.(attitude|pride)\\b", "^nativis"]
...
violence = ["^violence$", "\\bviolen(t|ce).(act|attitude|movement)", "(mass|collective) violence"]
...
discrimination = ["bull[yi]", "discriminat", "harrass", "humiliat"]
