...
df_plot$id <- apply(df_plot[, c("term1", "term2")], 1, function(x)paste0(sort(x), collapse = ""))
df_plot <- df_plot[!duplicated(df_plot$id), ]
write.csv(df_plot, "RTMR_Output/Keywords_cooc_single_prun.csv", row.names = FALSE)
...
df_plot$id <- apply(df_plot[, c("term1", "term2")], 1, function(x)paste0(sort(x), collapse = ""))
df_plot <- df_plot[!duplicated(df_plot$id), ]
write.csv(df_plot, "RTMR_Output/Keywords_cooc_wo_rad_single_prun.csv", row.names = FALSE)
